...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    // We can tell if this is the first time by checking for a key in NSUserDefualts
    NSUserDefaults *uDef = [NSUserDefaults standardUserDefaults];
    NSString *firstTime = [uDef stringForKey:@"firstTime"];
    
    // If there is no key uDef is Nil
    if (firstTime == Nil) {
        RHFirstTimeInstructionViewController *first = [[RHFirstTimeInstructionViewController alloc]
                                            initWithNibName:@"RHFirstTimeInstructionViewController"
                                            bundle:Nil];
        [self.window setRootViewController:first];
    }
    // If this is not the first time, load the application
    else
    {
...
- (NSManagedObjectModel *)managedObjectModel
{
    ...
    NSURL *modelURL = [[NSBundle mainBundle] URLForResource:@"RemoteHome" withExtension:@"mom"];
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    ...
    NSURL *storeURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"RemoteHome.sqlite"];
